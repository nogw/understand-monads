...
newtype Continuation x m r = Continuation {(>>-) :: (r -> m x) -> m x}

instance Functor (Continuation x m) where
  fmap f m = Continuation $ \_return -> m >>- \a -> _return (f a)

instance Applicative (Continuation x m) where
  pure r = Continuation $ \_return -> _return r
  (<*>) m x = Continuation $ \_return -> m >>- \f -> x >>- \x -> _return (f x)

instance Monad (Continuation x m) where
  return r = Continuation $ \_return -> _return r
  (>>=) m f = Continuation $ \_return -> m >>- \a -> f a >>- \b -> _return b
